...
| Fabula projection | The one-event-to-one-provisional-Scene-Card mapping used to lint a Fabula-only package. | `lintPackage` requires `scene_cards`; the projection lets the real checks run before segmentation exists. | `lintFabulaArc`; unrecoverable fields get a counted, deterministic substitution (ADR 0019). |
