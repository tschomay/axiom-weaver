...
| Term | What | Why | How |
| --- | --- | --- | --- |
| Fabula | The world and its events in chronological order. Owned by the author. | Separating "what happened" from "how it's shown" is what makes secrets and dramatic irony possible. | By hand, it's whatever the World Model seed and Scene Cards jointly establish; extraction/generation can now produce it standalone first — see Fabula-only package. |
...
| Term | What | Why | How |
| --- | --- | --- | --- |
| Fabula-only package | A Story Package with a World Model seed but empty `scene_cards`; Fabula produced ahead of Syuzhet. | Extraction and generation both stop before segmentation; #118 draws scene boundaries, not them. | Event list rides in a `_fabula` extra block, not a schema field (ADR 0019). |
| Fabula projection | The one-event-to-one-provisional-Scene-Card mapping used to lint a Fabula-only package. Never a publish-readiness check. | `lintPackage` requires `scene_cards`; the projection lets the real checks run before segmentation exists — but the real publish gate still lints the actual package, which fails regardless. | `lintFabulaArc`, its own result type, no `publishable` field; unrecoverable fields get a counted, deterministic substitution (ADR 0019). |
...
| Authoring run | The durable job behind Generate/Extract: a pipeline (arc generation or extraction) plus segmentation, as stages of one job. | An unsegmented package has no Scene Cards yet and isn't reviewable anywhere the author already looks, so there's no real pause to put between pipeline and segmentation. | Produces a `StoryPackage` that lands in the existing import preview; shares the read-time run loop's step seam, not its scene-shaped manifest (ADR 0021). |
