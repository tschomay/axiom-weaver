...
import { adjacencies, signalAvailability, type Adjacency, type SignalAvailability } from './signals';
...
export async function segmentFabulaPackage(
  ...
  progress(`${events.length} Fabula events, ${gaps.length} adjacencies`);
...
export function segmentMechanically(
  ...
  for (const [sceneId, entries] of carried.paysOff) {
    for (const entry of entries) {
      pairs.push({ fact_ref: entry.fact_ref, plant: entry.plant, payoff: sceneId, why: 'carried' });
    }
  }
  for (const scene of scenes) {
